...
def run_phase1_simulation(p_depth):
    # 1. Formulate the Max-Cut cost Hamiltonian for a 6-8 node graph
    G = create_graph(num_nodes=6)
    hamiltonian = get_cost_hamiltonian(G)
    
    # 2. Implement the QAOA ansatz with depth p=1 and p=2
    ...
    # 3. Run exact classical simulations using Statevector
